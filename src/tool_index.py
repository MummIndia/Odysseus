...
#
# Retrieval returns a top-8, and @playwright/mcp alone exposes 30 tools. In
# practice that meant the agent kept being handed browser_drop,
# browser_handle_dialog or browser_close while browser_navigate — the only
# tool that opens a page — never surfaced once. The model then correctly
# reported that it could not browse the web, and looked like it was refusing.
#
...
# are still connected and callable; they just no longer compete for a slot.
MCP_INDEX_DENIED = frozenset({
    # Low-level pointer primitives — browser_click/hover work off the
    ...
    # Session plumbing the agent should not be spending its budget on.
